...
export default class InfiniteListExample extends React.Component {
  ...
  unShare(id,folderId) {
    request.post(`/unshare`, { data: { id: id,folderId:folderId } }).then(
      (response)=>{
            ...
            renderItem={item => (
              <List.Item key={item.id} className={styles.list}>
                <List.Item.Meta
                  ...
                  title={item.type?<span className={styles.option} key="option" onClick={() => { this.edit(item.id,item.type) }}>{item.name}</span>:<Link to={'/folder/'+item.folderId}>{item.name}</Link>}

                  description={'创建时间'+item.createTime}
                />
                <div className={styles.optionBox}><span key="option1" onClick={() => { this.unShare(item.id,item.folderId) }}>取消分享</span></div>
