...
class UserController extends Controller {
  ...
  async myDoc() {
    ...
    await ctx.model.User.findById({
      ...
      let folder = person.folder.filter((item) => {
        ...
        return {
          ...
          share: item.share,
          doc: item.doc.filter((item) => item.delete == false&&item.rm==false).map((item) => {
            return {
              name: item.name,
              icon: item.icon,
              id: item._id,
      ...
      let doc = person.doc.filter((item) => {
        return item.delete == false&&item.rm==false;
      }).map((item) => {
        return {
          name: item.name,
      ...
      let json = {
        ...
        data: [...arr.slice(ctx.query.results * ctx.query.time - ctx.query.results, ctx.query.results * ctx.query.time)]
  ...
  async recently() {
    ...
    await ctx.model.User.findById({
      ...
      let folderDoc = person.folder.filter((item) => {
        ...
        return item.doc.filter((doc) => doc.delete == false&&item.rm==false).map((doc) => {
          return {
            name: doc.name,
            icon: doc.icon,
            id: doc._id,
      ...
      let docList=[];
      folderDoc.map((item)=>{docList.concat(item)});
      // console.log(docList);
  ...
  async findList() {
    ...
    await ctx.model.User.find().select('userName avatar userEmail doc folder').then(data=>{
      console.log(data);
      ...
      data.map((item)=>{
        ...
        return item.doc.filter((doc)=>{return doc.share&&!doc.delete&&!doc.rm}).map((doc)=>{return {name:doc.name,type:doc.type,icon:doc.icon,id:doc._id,createTime:doc.createTime,editTime:doc.editTime,userName:item.userName,userEmail:item.userEmail,avatar:item.avatar}}).concat(
          item.folder.filter((doc)=>{return doc.share&&!doc.delete&&!doc.rm}).map((doc)=>{return {name:doc.name,icon:doc.icon,folderId:doc._id,createTime:doc.createTime,editTime:doc.editTime,userName:item.userName,userEmail:item.userEmail,avatar:item.avatar,doc:doc.doc}}),
  ...
  //       let folder = data[0].folder.filter((item) => {
  //         return item.share == true&&item.rm==false;
  //       }).map((item) => {
  //         return {
  ...
  //       let doc = data[0].doc.filter((item) => {
  //         return item.delete == false&&item.rm==false;
  //       }).map((item) => {
  //         return {
  //           userName: item.userName,
